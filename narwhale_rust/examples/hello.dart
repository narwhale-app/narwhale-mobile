import '../lib/narwhale_rust.dart';

void main() {
  print(NarwhaleRust.greet('Rust'));
  ...
  String mnemonic = NarwhaleRust.mnemonic_from_entropy(entropy);
  print(mnemonic);

  // If we pass invalid number of bytes or invalid hex string it will throw an exception
  try{
    // invalid number of bytes should throw
    print(NarwhaleRust.mnemonic_from_entropy('313131313131313131313131313131'));    
  } catch (e) {
    print('Error: $e');
  }
  ...
  var root = NarwhaleRust.mnemonic_to_root_key(mnemonic, '');
  ...
  var xpub = NarwhaleRust.derive_xpub(xprv, "m/84'/0'/0'", 'bitcoin');
  print(xpub);
  ...
  var desc = NarwhaleRust.get_default_descriptors(xprv, 'bitcoin');
  print(desc);

  // Derive first 10 addresses of the wallet for bitcoin network
  var recv_desc = desc['recv_descriptor'];
  var addresses = NarwhaleRust.derive_addresses(recv_desc, 'bitcoin', 0, 10);
  print(addresses);

  // get descriptor for account 3, type - nested segwit.
  var nested_desc_acc_2 = NarwhaleRust.get_account_descriptors(xprv, 3, 'nested', 'bitcoin');
