...
const DYNAMIC_LIBRARY_FILE_NAME = 'libnarwhale_rust.so';

class NarwhaleRustException implements Exception {
  String _message = 'Rust bindings error';
  NarwhaleRustException([String message = 'Rust bindings error']) {
    _message = message;
  }
  @override
  String toString() {
...
class NarwhaleRust {
  static final NarwhaleRustBindings _bindings =
      NarwhaleRustBindings(NarwhaleRust._loadLibrary());

  static NarwhaleRustBindings getBindings() {
    return _bindings;
  }
  ...
  static DynamicLibrary _loadLibrary() {
    ...
    if (Platform.environment.containsKey('FLUTTER_TEST')) {
      var hostLib = './rust/target/debug/libnarwhale_rust';
      if (Platform.isLinux) {
        hostLib += '.so';
  ...
  // if error occured - throws NarwhaleRustException
  static Map<String, dynamic> _decode_result(String result) {
    var data = jsonDecode(result);
    if (data['status'] == 'error') {
      throw NarwhaleRustException(data['message']);
    }
    return data;
  }
  ...
  ///
  /// ```dart
  /// String mn = NarwhaleRust.mnemonic_from_entropy('31313131313131313131313131313131');
  ...
  /// Throws a [NarwhaleRustException] if [mnemonic] is invalid or network is unknown.
  ///
  /// Returns an object {'fingerprint': '4-bytes-in-hex', 'xprv': 'root private key'}
  ///
  /// ```dart
  /// var root = NarwhaleRust.mnemonic_to_root_key('couple maze era give basic obtain shadow change couple maze era glide', '');
  ...
  ///
  /// Throws a [NarwhaleRustException] if [xprv] key is invalid or [path] is formatted incorrectly.
  ///
  ...
  ///
  /// ```dart
  /// String xpub = NarwhaleRust.derive_xpub('xprv9s21ZrQH143K48dWHMyaUE3yzA6KV4MkKytF2unviMieqjN8MtfXSu6WbM29w8UngGtaAWEe65u1SVcPBxoMZLasQXw6MMuZSYWb1QDAbZm', 'm/84h/0h/0h', 'bitcoin');
  ...
  /// [network] can be 'bitcoin', 'testnet', 'signet' or 'regtest'.
  ///
  /// Throws a [NarwhaleRustException] if [xprv] is invalid or network is unknown.
  ///
  ...
  /// [network] can be 'bitcoin', 'testnet', 'signet' or 'regtest'.
  ///
  /// Throws a [NarwhaleRustException] if [descriptor] is invalid, network is unknown or indexes are out of range.
  ...
  static List<dynamic> derive_addresses(
    ...
    if(start < 0 || end < 0 || start >= 0x80000000 || end >= 0x80000000){
      throw NarwhaleRustException('indexes must be in range [0, 0x80000000)');
    }
  ...
  static void sayHi() {
    print('Hello from narwhale_rust (Dart side)');
  }
}
